  ...
  return (
    <div className="shadow-4xl  active:scale-[0.990] duration-200 rounded-lg xs:max-xsm:px-1 xs:xsm-sm:py-0 p-2" >
      ...
      xxl:min-w-[400px] xxl:min-h-32   relative  flex items-center space-x-4   xs:max-sm:space-x-2 xxl:gap-x-6 ">
        <div className="">
              <img className="rounded-full bg-cover border-2 min-w-14 max-w-14 min-h-14 max-h-14 border-indigo-500" src={`http://localhost:5000/uploads/users-profile/${profile??"no-avator.png"}`} alt={`${name}`} />
        </div>
        <div className="flex flex-col space-y-2">
            ...
            <p className="xs:max-sm:text-[12px] md:max-lg:text-[13px] text-sm capitalize bg-indigo-500 px-1 rounded-sm text-white">{role=="resident"?"project manager":role=="maintainence"?"employee":"admin"}</p>
